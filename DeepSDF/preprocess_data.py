...
if __name__ == "__main__":
    ...
    args = arg_parser.parse_args() #定義した引数を代入

    deep_sdf.configure_logging(args) #deepsdfモジュール内のログ設定

    additional_general_args = []

    deepsdf_dir = os.path.dirname(os.path.abspath(__file__))
    if args.surface_sampling:#surfaceが指定されたらplyファイルを出力する
    ...
    with open(args.split_filename, "r") as f:
        split = json.load(f)#読み取ったjsonファイルを格納
    ...
    for class_dir in class_directories:
        ...
        print("instance_dirs", instance_dirs)
        ...
        for instance_dir in instance_dirs:
            ...
            try:
                ...
                if args.surface_sampling:
                    ...
                    print("normalization_param_target_dir", normalization_param_target_dir)
                    ...
                    normalization_param_filename = os.path.join(
                        normalization_param_target_dir, instance_dir + ""
                    )
                    specific_args = ["-n", normalization_param_filename]

                meshes_targets_and_specific_args.append(
                    (
                        # os.path.join(shape_dir, mesh_ftexture_filename),
                        mesh_filename,
                        processed_filepath,
                        specific_args,
                    )
                )
                print("meshes_targets_and_specific_args.append", meshes_targets_and_specific_args)
